...
class SportsManager: ObservableObject {
    ...
    @Published var currentSportType: SportActivity?
    @Published var currentSportTypeString: String?
    @Published var currentSportState: Int = 0
    @Published var currentCalorie: Int = 0
    ...
    func updateData() {
        QCSDKManager.shareInstance().currentSportInfo = { info in
                ...
                self.currentSportTypeString = info.description
                
                print("State:", info.state)
                
                print("Duration:", info.duration)
                self.currentSeconds = info.duration
                
                print("HR:", info.hr)
                self.currentHeartRate = info.hr
                
                print("Steps:", info.step)
                self.currentSteps = info.step
                
                print("Calorie:", info.calorie)
                self.currentCalorie = info.calorie
                
                print("Distance:", info.distance)
                self.currentDistance = info.distance
    ...
    func pauseSport(type: OdmSportPlusExerciseModelType, completion: (() -> Void)? = nil) {
        QCSDKCmdCreator.operateSportMode(
            with: type,
    ...
    func resumeSport(type: OdmSportPlusExerciseModelType, completion: (() -> Void)? = nil) {
        QCSDKCmdCreator.operateSportMode(
            with: type,
    ...
    func stopSport(type: OdmSportPlusExerciseModelType, completion: (() -> Void)? = nil) {
        QCSDKCmdCreator.operateSportMode(
            with: type,
